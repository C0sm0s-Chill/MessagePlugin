...
namespace MessagePlugin {
  ...
  public class MessagePlugin : TerrariaPlugin {
    ...
    public override string Author {
      get { return "Created by Lmanik update by C0sm0s"; }
    }
    ...
    public static void SendMessage(string to, string from, string text) {
      ...
      List<TSPlayer> players = TSPlayer.FindByNameOrID(to);
      if (players.Count == 1) {
    ...
    public static void Msg(CommandArgs args) {
      ...
      switch (cmd) {
        ...
        default: {
            if (args.Parameters.Count > 1) {
              UserAccount userPlr = TShock.UserAccounts.GetUserAccountByName(args.Parameters[0]);
